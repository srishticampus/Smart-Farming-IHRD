...
class SignupActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun checkAllFields(): Boolean {
        ...
        if (fullName.isEmpty()) {
            ...
            isValid = false
        } else if (fullName.replace(" ", "").length < 3) {
        ...
        if (password.isEmpty()) {
            ...
            binding.password.error = "Password must be 6 to 8 characters"
            isValid = false
        ...
        if (isValid) {
            ...
            val currentDrawable = binding.imgProfile.drawable

            if (selectedImageUri == null && currentDrawable.constantState == defaultDrawable?.constantState) {
